...
async function main() {
  ...
  const marStart = '2026-03-01T00:00:00-03:00';

  const before = {
    readings_recorded_at: await countRows(supabase, 'readings', (q) =>
      q.gte('recorded_at', febStart).lt('recorded_at', marStart)
    ),
    sensor_device_timestamp: await countRows(supabase, 'sensor_readings', (q) =>
      q.gte('device_timestamp', febStart).lt('device_timestamp', marStart)
    ),
    sensor_ingested_at: await countRows(supabase, 'sensor_readings', (q) =>
      q.gte('ingested_at', febStart).lt('ingested_at', marStart)
    ),
  };
  console.log('Before:', before);

  await deleteWindowed(supabase, 'delete readings feb', 'readings', 'recorded_at', febStart, marStart);
  await deleteWindowed(
    supabase,
    'delete sensor_readings feb by device_timestamp',
    'sensor_readings',
    'device_timestamp',
    febStart,
    marStart
  ...
  await deleteWindowed(
    supabase,
    'delete sensor_readings feb by ingested_at',
    'sensor_readings',
    'ingested_at',
    febStart,
    marStart
