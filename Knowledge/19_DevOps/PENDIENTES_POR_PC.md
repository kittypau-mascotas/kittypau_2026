...
| 4 | `002-registro-flow-unificado` — Fases 5-8 de `tasks.md`: Ficha Detallada (Salud/Alimentación) en `/pet` + círculo rojo en el menú (US6), columna única/radios en el resto del flujo (US5), scroll generalizado (US3), polish final | [[29_Specs/002-registro-flow-unificado/tasks]] | MVP (US1+US2) y Registro Básico ampliado (US4) ya en `main` |
...
|---|---|---|---|
| 2026-08-16 | `002-registro-flow-unificado` MVP (US1+US2) + Registro Básico ampliado (US4) — registro fusiona cuenta+perfil+mascota en el paso 1 (3 pasos totales, antes 4), correo de confirmación personalizado (nombre+mascota) vía Supabase Auth, migración aditiva en `pets` aplicada y verificada en producción (`sex`, `microchip_number`, `birth_date`/`intake_date`, `health_profile`/`feeding_profile` jsonb). Probado end-to-end con cuenta real (`frentecalamari@gmail.com`) y con Playwright. Adopción de spec-kit como herramienta (specs viven en `Knowledge/29_Specs/`, no en `specs/`) | PC de Mauro, autorizado por Mauro | [[29_Specs/002-registro-flow-unificado/spec]] |
