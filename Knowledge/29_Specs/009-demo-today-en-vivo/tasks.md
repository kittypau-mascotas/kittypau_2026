...
- [X] T024 [US3] **CHECKPOINT Principio III**: obtener OK explícito de Mauro para cambiar el schema de `demo_ingresos`. → Mauro aprobó ("Sí, apruebo la migración")
...
- [X] T026 [US3] Aplicar la migración a Supabase — APLICADA a producción `zjdyhpntftgaynchqwfk` el 2026-09-10 (vía `pg` + `DATABASE_URL`, en transacción). Verificado: `email` nullable, columnas `visitor_id`/`pet_type`, índice parcial `demo_ingresos_visitor_uniq`, función `record_demo_ingreso_v2` (grants revocados a anon/authenticated), smoke-test de las 2 ramas OK
