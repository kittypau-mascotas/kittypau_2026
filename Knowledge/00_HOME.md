...
- **El JS se despliega solo con cada push a `main` (Vercel), los recursos nativos del APK NO** (plugins, íconos, permisos) — necesitan un APK nuevo compilado e instalado. Ver [[29_Specs/SPEC_06_Mobile_APK_2026]]. Desde 2026-09-15 hay una forma de compilarlo sin la PC de Mauro: `.github/workflows/build-android-apk.yml` (CI manual, `gh workflow run`) — ver [[19_DevOps/README_DevOps]]. **La PC de Mauro también puede compilar localmente** (confirmado 2026-09-15, sesión posterior): ya tiene Android SDK + JDK 21 (JBR de Android Studio) instalados, solo falta `$env:JAVA_HOME = "C:\Program Files\Android\Android Studio\jbr"` antes de `gradlew` — más rápido para iterar que disparar el workflow de CI. Detalle en [[19_DevOps/PENDIENTES_POR_PC]] #3.
