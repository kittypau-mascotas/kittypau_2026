...
- `G3` Step line: telemetra discreta o cambios por evento.
- `G4` Sparkline compacta: tarjeta KPI con micro-historial.
- `G5` Barras por ventana: agregados por minuto/hora.
- `G6` Gauge semicircular: batera, salud de sensor, conectividad.
...
- `S1` Clean Minimal: ejes finos, lnea principal, fondo neutro.
...
- `A4` Eje Y fijo: rango operacinal (recomendado para comparar dias).
- `A5` Etiquetas grandes: prioriza lectura rpida en mobile.
...
- `M4` Subtle glow: brillo leve en lnea activa.
...
- **Comida + Agua**: `G1 + S2 + A2 + A3` — eje X lneal 0-24h (ciclo 6am-6am), imagen como punto, `detectIntakeSessions` para sesiones de consumo, plugin de fondo `fondo.png`.
