...
- En `signUp`, `emailRedirectTo` apunta a: `/loginregister=1&verified=1`

### Variantes de confirmacion soportadas
- PKCE: `/loginregister=1&code=...`
  - Se usa `exchangeCodeForSession(code)` y luego se avanza a onboarding.
- OTP/hash: `/loginregister=1&type=signup&token_hash=...`
  - Se usa `verifyOtp({ type, token_hash })` y luego se avanza a onboarding.
- Mensaje simple: `/loginregister=1&verified=1`
...
- Si `signUp` no entrega `session` (lo normal con confirmacion por correo), el Step 1 muestra:
  - "Revisa tu correo (y spam) para confirmar... Cuando confirmes, volveras aqu y pasaremos automaticamente al paso Usuario."
...
## Objetivo
En el primer ingreso con usuario + mascota + dispositivo listos, mostrar un modo gua para aprender a usar Kittypau.
...
- Se mantiene vlidación real por etapa (no solo navegación visual).
