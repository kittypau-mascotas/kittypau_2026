# KittyPau — Capas de Datos, Analítica/ML e IA (inventario de variables)
...
Nota: la **fuente canónica** de arquitectura de datos/ML es `Docs/KittyPau_Arquitectura_Datos_v3.md`. Este documento se mantiene como **inventario rápido** (variables + “dónde vive cada cosa”) y debe evitar contradicciones con el schema real (`Docs/SQL_SCHEMA.sql`).
...
En el proyecto existen estructuras analíticas tipo “sesión” en el **proyecto `supabase-analytics/`** (tablas `public.pet_sessions` y `public.pet_daily_summary`):
...
### `supabase-analytics: public.pet_sessions` / `supabase-analytics: public.pet_daily_summary` (analítica por mascota)
