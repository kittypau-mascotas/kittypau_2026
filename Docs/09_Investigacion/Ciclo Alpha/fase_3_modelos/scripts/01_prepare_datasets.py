...
def main() -> None:
    ...
    # Exp 09B: copiar ts_val.parquet para calibración de threshold por período
    ts_val_src = PHASE2_TRAIN_DIR / "ts_val.parquet"
    if ts_val_src.exists():
        shutil.copy2(ts_val_src, MODEL_A_DIR / "ts_val.parquet")
        print(f"[OK] ts_val.parquet copiado a {MODEL_A_DIR}")
    else:
        print("[AVISO] ts_val.parquet no encontrado en Fase 2 — calibración por período no disponible")
