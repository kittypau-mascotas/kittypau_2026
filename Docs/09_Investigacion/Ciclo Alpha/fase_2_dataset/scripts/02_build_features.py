...
def main() -> None:
    ...
    # Exp 09 — Normalización de cadencia a 30 s
    n_pre = len(df)
    df = resample_to_uniform(df, target_s=RESAMPLE_TARGET_S)
    n_post = len(df)
    print(f"[OK] Resampleo a {RESAMPLE_TARGET_S}s: {n_pre:,} -> {n_post:,} filas "
          f"({n_pre - n_post:,} consolidadas, {n_post / n_pre * 100:.1f}% retenidas)")
