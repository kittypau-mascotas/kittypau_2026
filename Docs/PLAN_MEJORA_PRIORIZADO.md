...
2. Reducir duplicacion visual entre `today`, `story`, `pet`, `bowl` y `settings` usando componentes compartidos cuando el patron se repite.
...
4. Registrar alertas de gaps, lecturas anomales y saltos extremos. Ya existe auditoria de gaps de lectura en el webhook y el admin los cuenta con un panel de acciones separado.
