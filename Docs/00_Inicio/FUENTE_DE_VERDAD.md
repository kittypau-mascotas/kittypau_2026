...
- `Docs/`: solo documentos canonicos o de operacin activa.
- `.github/` y `.husky/`: automatizacion y calidad.

## 2) Que es legacy o esta archivado

- `Analisis_Estadistico_ML_IA/`: workspace de anlisis ya ignorado.
...
- `tools/` y `Test Graficos/`: trabajo local / tilidades no productivas.
...
- `Docs/investigacion/AUDITORIA_KPCL0036_ERROR_PESO_SIN_BATERIA.md` como auditoria canonica del CSV `Docs/investigacion/kpcl0036_error_peso_sinbatera.csv`, cuando se necesite revisar peso bruto sin tara aplicada y su diagnostico tecnico.
...
- `Docs/investigacion/SQL_VALIDACION_KPCL0036_TARE_FILL.sql` como consulta canonica para vlidar secuencias de tare, peso de plato, inicio de llenado a las `20:06:55+00` y termino de llenado en `KPCL0036`.
...
- `Docs/investigacion/AUDITORIA_KPCL0036_ERROR_PESO_SIN_BATERIA.md` como referencia de auditoria para la misma secuencia cuando se compare `KPCL0036` con `KPCL0034`.
...
- Categoria manual canonica al `2026-04-07 00:17:41+00:00` para `KPCL0034` con `inicio_alimentacin`, registrada desde la nueva capa de botones del plato.
- Categoria manual canonica al `2026-04-07 00:20:41+00:00` para `KPCL0034` con `termino_alimentacin`, registrada desde la nueva capa de botones del plato.
- `Docs/investigacion/SQL_EXPORT_KPCL0034_KPCL0036_EXPERIMENTO.sql` como export canonico del tramo de experimento compartido de ambos devices, desde 5 minutos antes de la foto operativa hasta ahora.
- `Docs/investigacion/AUDITORIA_KPCL0034_KPCL0036_PRUEBA_SIN_CARGADOR.md` como auditoria canonica del CSV bruto del experimento compartido sin cargador.
- Interpretacion canonica de ese CSV: se analiza la variacion de peso sin gato ni interrupcion externa directa del sensor; cualquier cambio debe priorizar hipotesis de medicion, alimentacin, mecanica o calibracion.
...
- Ciclos de batera en `public.device_battery_cycles` cuando exista telemetra `battery_*` o cuando se carguen observaciones manuales canonicas para calculos historicos, como los ciclos de `KPCL0034` y `KPCL0036`, incluyendo el inicio manual de autonoma de `KPCL0036` tras desconectar el cargador.
...
- Pruebas controladas de peso y carga para `KPCL0034` y `KPCL0036` como base comun de auditoria antes de derivar reglas automticas.
...
- La duracion real de batera no se puede inferir sin telemetra de energa, salvo los tramos manuales canonicos que se guarden expresamente como referencia de anlisis.
...
- `battery cycle`: periodo de carga o uso con batera, cuando exista telemetra de energa.
- `battery state`: campo de telemetra esperado (`charging`, `battery_only`, etc.).
- `battery source`: fuente de energa detectada (`external_power`, `battery`, etc.).
- `battery cycles`: nombre recomendado para la tabla historica de carga y autonoma.
