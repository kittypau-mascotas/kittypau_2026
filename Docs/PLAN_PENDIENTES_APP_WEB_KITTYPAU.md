...
## Objetivo
Ordenar el trabajo pendiente para mejorar experiencia de usuario, estabilidad operativa y valor AIoT en `web + app` sin perder la lgica real `mascota <-> KPCL`.

## Estado actual resumido
- Lo critico sigue siendo la coherencia de `/today`, la robustez de `/api/readings` y el deploy.
- La batera real no esta disponible en telemetra historica de KPCL0034.
...
1. Estabilizar lgica de seleccion en `/today`.
...
- Mantener fallback cuando no existan columnas nuevas de batera.
...
## P1 - Alto impacto (siguente ciclo)
...
- Validar formulas de batera estimada vs real y exponer fuente (`real/estimated`) en UI.
...
1. Story y narrativa.
- Convertir timeline en insights accinables (tendencias semanales/mensuales, cambios de habito).
- Alnear copy con posicionamiento PetTech AIoT.
...
- Deploy a Vercel con checklist completo y rollback vlidado.
