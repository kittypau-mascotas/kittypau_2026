...
- Confirmacion por correo: al vlidar, el flujo vuelve al pop-up y continua en Paso 2 (Usuario).

### Confirmacion de cuenta (Supabase)
- Redirect configurado en `signUp`:
  - `/loginregister=1&verified=1`
- Variantes soportadas al volver desde el correo:
  - PKCE: `/loginregister=1&code=...` -> `exchangeCodeForSession(code)`
  - OTP/hash: `/loginregister=1&type=signup&token_hash=...` -> `verifyOtp({ type, token_hash })`
  - Simple: `/loginregister=1&verified=1` (abre el pop-up; al existir sesin, avanza a Paso 2)
### Persistencia de sesin
- El cliente browser de Supabase usa `persistSession: true` y `autoRefreshToken: true`.
- La app guarda tokens de trabajo en `src/lib/auth/token.ts` y los refresca cuando hace falta.
- Regla UX vigente:
  - si el usuario ya inicio sesin en ese dispositivo, no se debe pedir login otra vez al abrir la app;
  - `/` debe resolver hacia `/today` o `/admin` segn la cuenta autenticada;
  - `/login` debe redirigir a la app si ya existe una sesin vlida y no hay flujo pendiente de registro/reset/verificacin;
  - el cierre de sesin solo ocurre con la accin explcita `Cerrar sesin`.
...
5. `GET/POST /api/devices`
   ...
   - Internamente usa RPC `link_device_to_pet` (operacin atomica).
   - Paginacion opcional: `limit=20&cursor=2026-02-08T00:00:00Z`.
     - Si hay `limit` o `cursor`, respuesta: `{ data, next_cursor }`.

6. `GET /api/readingsdevice_uuid=...`
...
- Antes de llamar APIs, el frontend intenta obtener un token vlido:
  - Si el `access_token` est por expirar, se renueva usando `refresh_token`.
...
- Esto permite usar herramientas externas u orgenes distintos (p.ej. pruebas en otro PC/local) con `Authorization` o `x-bridge-token`.
...
- Fallback: polling cada X segndos si Realtime falla.
...
- Mantener el API liviano (vlidación y escritura en DB).
...
- 400 si payload es invalido o falla una vlidacion.
...
6. POST /api/pets
   ...
   - 401 si falta token.
## Nota sobre vlidaciones
Los enums de `origin`, `living_environment`, `size`, `activity_level`, `age_range`, `alone_time` se vlidan en frontend/backend.
El SQL actual solo impone constraints en `type` y `pet_state` para `pets`.

## Alneación DB  API (estado actual)
...
- **En API (vlidación adicional, no estricta en DB):**
