...
## Flujo de datos (telemetrÌa)
1. ESP32 publica MQTT en HiveMQ.
2. Raspberry Bridge escucha MQTT y reenvÌa a Vercel.
3. API valida el token y guarda lectura en Supabase.
4. Supabase Realtime actualiza el dashboard.

### Transformaciones analÌticas (recomendado)
- En ingesta server-side (despuÈs de validar y antes de persistir): aplicar `log10(x + 1)` a variables skewed (ej. `weight_grams`, `water_ml`, intervalos).
- Persistencia `raw + transformado`: es **recomendaciÛn**, pero depende del schema (hoy `public.readings` no incluye `*_log` en `Docs/SQL_SCHEMA.sql`).
- Fourier/FFT **no va** en el bridge/webhook: va en un worker/servicio analÌtico (batch o microservicio).
