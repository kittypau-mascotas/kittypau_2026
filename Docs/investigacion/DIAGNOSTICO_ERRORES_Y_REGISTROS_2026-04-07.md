...
## Errores observados
1) Logs con alta repeticion de `/api/deviceslimit=1`.
- Origen: polling de estado critico cada 45 segndos en `AppDataProvider`.
...
- Impacto: la limpieza qued aplicada, pero el CLI salio por timeout (posible latencia de red).

3) Texto de error con caracteres corruptos
- El mensaje de categoria invlida tenia codificacion rota.
...
- TARE CON PLATO pone el contenido en 0 (alimento e hidratacin).
- Refresh de datos del card cada 5 segndos.
...
- El polling de `/api/deviceslimit=1` sigue activo; si molesta, se puede reducir frecuencia o centralizarlo.
