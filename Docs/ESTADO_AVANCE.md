...
- Confirmacion de correo (Supabase) soportada para volver al popup y continuar:
  - `/loginregister=1&code=...` (PKCE) -> `exchangeCodeForSession(code)`
  - `/loginregister=1&type=signup&token_hash=...` (OTP/hash) -> `verifyOtp({ type, token_hash })`
  - `/loginregister=1&verified=1` (abre popup; al existir sesion, avanza automaticamente)
- Regla UX: si el usuario ya esta vlidado (hay sesion Supabase), el popup salta el Paso 1 y entra directo a Paso 2 (Usuario).
...
- `GET /api/devices` dispara health-check en segndo plano (best-effort) para ejecutar detección operativa sin cron en plan Hobby.
...
- API: soporte CORS en `/api/*` (preflight `OPTIONS`) para evitar "Failed to fetch" en herramientas/or?genes externos.
- Storage: referencia de policies para `kittypau-photos` en `Docs/SQL_STORAGE_POLICIES_kittypau_photos.sql` (configurar v?a Dashboard).
- Auth: refresh silencioso mejorado (se renueva access_token cuando est? expirando) + helper `authFetch()` para reintentar una vez ante `401`.
...
- Raspberry Bridge: pendiente de vlidar 24/7 (systemd)
...
- Login/register: vlidación básica + recuperación de contraseña + banner de verificación.
...
- /bowl refinado: estado con diagnóstico de conexión y accines futuras.
- /settings refinado: vlidaciones de perfil antes de guardar.
...
- Onboarding UI en una sola vista con pasos guados y vlidaciones de perfil.
- Popup de registro implementado desde /login con flujo account + onboarding.
- Backend hardening v1 completo (errores, rate limit, vlidaciones, auditoria, RPC, indices, cleanup).
...
- Docs MQTT alneados a topics `+/SENSORS` (arquitectura y Raspberry CLI).
...
- Limites de payload y rangos vlidados (weight_kg, battery_level, readings limit).
...
- Webhook vlidado con devices.id (UUID) y device_id (KPCL) y valores string normalizados.
...
- E2E vlidado (Auth -> Pets -> Devices -> Webhook -> Readings).
...
- [x] Diseño  Producto (lneamientos y componentes definidos).

## Riesgos conocidos
- Refresh token no implementado en UI (pendiente siguente iteracion).

## Verificaciones cerradas (operacin)
- [x] Schema cache refrescado en Supabase.
- [x] Constraints de onboarding aplicadas (2026-02-07).
- [x] Variables de entorno vlidadas entre Vercel y Raspberry.
...
## Resumen por módulo
- UI: onboarding guado con vlidaciones, tooltips visuales, popup de registro y mejoras en today (selector device + refresh + frescura).
...
- Navbar app renovado:
  ...
  - menú de accines movido al trigger de perfil (avatar + nombre).
  - accines en menú: `Ajustes`, `Editar perfil`, `Cerrar sesión` (+ `Dashboard admin` si corresponde).
...
- Invlidacion de cache en `admin/health-check`, `bridge/heartbeat` y `bridge/health-check`.
...
- `% Supabase tilizado` ahora calcula total real:
...
- `ChartJS.register(...)` centralizado una sola vez; eliminados ~180 lneas duplicadas de `/bowl`.
...
- `Cache-Control: private, max-age=N, stale-while-revlidate=N` en GET de cada endpoint:
  - profiles: 60s / 300s, pets: 60s / 300s, devices: 30s / 120s, account/type: 300s / 3600s.
- Columnas explcitas en `readings` SELECT (elimina transferencia de campos innecesarios).
...
- Cards de alimento/hidratacin en `/today`:
...
- Ajuste de interaccin:
  - animacion/interaccin de plato se mantiene solo en el plato grande del login.
