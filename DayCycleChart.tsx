...
export const DayCycleChart: React.FC<Props> = ({ data, height = 320, compact = false }) => {
  ...
  useEffect(() => {
    ...
    svg.on("mousemove", (event) => {
      ...
      if (mx >= 0 && mx <= innerWidth) {
        ...
        if (d) {
          focusCircle.attr("cx", x(d.time)).attr("cy", y(d.weight)).style("opacity", 1);
        }
      }
    })
    .on("mouseleave", () => {
    ...
    // Eje X
    if (!compact) {
      const xAxis = d3.axisBottom(x).ticks(8).tickFormat(d3.timeFormat("%H:%M") as any);
      g.append("g")
