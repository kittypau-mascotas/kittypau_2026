...
/**
 * Mapa del archivo (agregado 2026-08-12, extracción por componentes en
 * curso desde el mismo día — A-C1 en Knowledge/29_Specs/SPEC_02_UIUX_Mejoras.md,
 * ya no "dejado de lado", Mauro pidió avanzar explícitamente). Grepear el
 * texto del `<h2>` (acá o dentro de `admin/_components/`) para saltar
 * directo a una sección, no confiar en el número de línea.
 *
 * Extraído a `admin/_components/` (presentacional, el cálculo de los datos
 * sigue acá — mismo patrón que `DayNightTimelineCard` de /today):
 * "Avisos críticos" (`avisos-criticos-card.tsx`), "KPI ejecutivos"
 * (`kpi-ejecutivos-card.tsx`), "Modelos de Negocio" + "4) Escalamiento" +
 * "Valorización SaaS" (`modelos-negocio-card.tsx`), "2) Auditoría e
 * Integridad de Datos" + "Estado de registro" + "Registros pendientes
 * recientes" (`auditoria-card.tsx` — batch 2, 2026-08-13), "3) Infraestructura
 * y Telemetría" + "Estado de bridges" + "Estado KPCL (online/offline)"
 * (`infraestructura-card.tsx` — batch 2, 2026-08-13), "Suite de Tests Admin"
 * (`tests-admin-card.tsx` — batch 3, 2026-08-15, incluye su propio
 * `ADMIN_TEST_CATALOG`, ya no vive acá), `SectionStatusCard`
 * (`section-status-card.tsx` — compartida por Operación/Auditoría/
 * Infraestructura/Finanzas/Tests, usada desde acá con `import`, no está
 * definida en este archivo).
 *
 * Todavía inline acá (`<h2>`, en orden de aparición):
 * "1) Operación del Servicio" → "Resumen de Finanzas" → "Finanzas" →
 * "Continuidad KPCL" → "Tablas y Vistas (Uso Aproximado)" →
 * "Resumen de incidentes (24h)" → "Panel de acciones" →
 ...
 * `formatClp`, `formatJpy` — `formatAgo` está duplicado en
 * `section-status-card.tsx` y en `auditoria-card.tsx`, deliberado, ver esos
 * archivos), `resolveKpclCatalog`.
 */
...
import SectionStatusCard, {
  type SectionStatus,
} from "./_components/section-status-card";
import AvisosCriticosCard from "./_components/avisos-criticos-card";
import KpiEjecutivosCard from "./_components/kpi-ejecutivos-card";
import ModelosNegocioCard from "./_components/modelos-negocio-card";
import AuditoriaCard from "./_components/auditoria-card";
import InfraestructuraCard from "./_components/infraestructura-card";
import TestsAdminCard from "./_components/tests-admin-card";
...
export default function AdminPage() {
  ...
  const [kpclHourlyStatus, setKpclHourlyStatus] = useState<
    KpclDeviceHourlyStatus[]
  >([]);
  ...
  const [supabaseStorage, setSupabaseStorage] =
    useState<SupabaseStorageSummary | null>(null);
  const [vercelUsage, setVercelUsage] = useState<VercelUsageSummary | null>(
    null,
  );
  ...
  const [financeBreakdown, setFinanceBreakdown] =
    useState<FinanceBreakdown | null>(null);
  const [financeBreakEven, setFinanceBreakEven] =
    useState<FinanceBreakEven | null>(null);
  ...
  const [testRunnerMessage, setTestRunnerMessage] = useState<string | null>(
    null,
  );
  ...
  const runHealthCheck = useCallback(async () => {
    if (permissions && !permissions.can_run_health_check) {
      setHealthCheckStatus((prev) => ({
        ...prev,
        message: "Tu rol no permite ejecutar el health-check.",
      }));
      return;
    }
    ...
    setHealthCheckStatus((prev) => ({ ...prev, running: true, message: null }));
    try {
      ...
      if (!res.ok) {
        throw new Error(
          payload?.error ?? "No se pudo ejecutar el health-check.",
        );
      }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!mounted) return;
        setPermissions(
          (payload.permissions ?? null) as AdminPermissions | null,
        );
        ...
        setVercelUsage(
          (payload.vercel_usage ?? null) as VercelUsageSummary | null,
        );
        setFxReference((payload.fx_reference ?? null) as FxReference | null);
        setFinanceSummary(
          (payload.finance_summary ?? null) as FinanceSummary | null,
        );
        setFinanceBreakdown(
          (payload.finance_breakdown ?? null) as FinanceBreakdown | null,
        );
        setFinanceBreakEven(
          (payload.finance_break_even ?? null) as FinanceBreakEven | null,
        );
        setFinancePlans((payload.finance_plans ?? []) as FinancePlan[]);
        setDbObjectStats((payload.db_object_stats ?? []) as DbObjectStat[]);
        setKpclCatalog(
          (payload.kpcl_catalog ??
            DEFAULT_KPCL_COST_CATALOG) as KpclCatalogPayload,
        );
        ...
        try {
          ...
          if (testsRes.ok) {
            ...
            setTestHistory(
              (testsPayload.history ?? []) as AdminTestHistoryItem[],
            );
          }
        } catch {
  ...
  const criticalAlerts = useMemo(() => {
    ...
    if (summary.kpcl_offline_devices > 0) {
      alerts.push(
        `${summary.kpcl_offline_devices} dispositivo(s) KPCL offline.`,
      );
    }
    ...
    if (lowBatteryCount > 0) {
      alerts.push(
        `${lowBatteryCount} dispositivo(s) KPCL con batería crítica (≤5%).`,
      );
    }
    return alerts;
  ...
  const continuityChart = useMemo(() => {
    ...
    const hourWidth =
      visibleHours > 1 ? plotWidth / (visibleHours - 1) : plotWidth;
    ...
    const rangeLabel =
      referencePoints.length > 1
        ? `${new Date(
            referencePoints[startIndex]?.ts ?? referencePoints[0].ts,
          ).toLocaleDateString("es-CL", {
            day: "2-digit",
            month: "2-digit",
          })} - ${new Date(
            referencePoints[referencePoints.length - 1].ts,
          ).toLocaleDateString("es-CL", {
            day: "2-digit",
            month: "2-digit",
          })}`
    ...
    const lastSeenByDevice = new Map(
      rows.map((row) => [
        row.device_id,
        formatLastSeenShort(
          kpclByDeviceId.get(row.device_id)?.last_seen ?? null,
        ),
      ]),
    );
  ...
  const selectedKpclCost = useMemo(() => {
    ...
    const monthlyRuntimeUsd =
      selectedKpclCatalog.maintenance_monthly_usd +
      selectedKpclCatalog.power_monthly_usd;
  ...
  const selectedKpclRuntimeSim = useMemo(() => {
    const mbByDevice = new Map(
      kpclTotalsOrigin.map((row) => [
        row.device_id,
        Number(row.data_mb_total ?? 0),
      ]),
    );
    const hoursByDevice = new Map(
      kpclTotalsOrigin.map((row) => [
        row.device_id,
        Number(row.online_hours_total ?? 0),
      ]),
    );
    ...
    const totalMb = Math.max(
      0.0001,
      kpclTotalsOrigin.reduce(
        (acc, row) => acc + Number(row.data_mb_total ?? 0),
        0,
      ),
    );
    const totalHours = Math.max(
      0.0001,
      kpclTotalsOrigin.reduce(
        (acc, row) => acc + Number(row.online_hours_total ?? 0),
        0,
      ),
    );
    ...
    const vercelUnitUsd =
      vercelBudgetMonthly * (mbShare * 0.7 + hourShare * 0.3);
    ...
    selectedKpclId,
    financePlans,
    selectedKpclCost.monthlyRuntimeUsd,
  ]);
  ...
  const kpclRuntimeByDevice = useMemo(() => {
    ...
    const out = new Map<
      string,
      { mb: number; hours: number; hivemqUsd: number; vercelUsd: number }
    >();
  ...
  const kpclFinancialRows = useMemo(() => {
    return kpclDevices.map((d) => {
      ...
      const unitUsd = components.reduce(
        (acc, c) => acc + c.qty * c.unit_cost_usd,
        0,
      );
  ...
  const executiveKpis = useMemo(() => {
    ...
    const bridgeOnlinePct =
      summary.bridge_total > 0
        ? Math.round((summary.bridge_active / summary.bridge_total) * 100)
        : 0;
    ...
    return [
      ...
      },
      {
        ...
        aux: `${summary.offline_events_last_24h} eventos`,
      },
      {
        key: "reg",
  ...
  const operationSectionStatus = useMemo<SectionStatus>(() => {
    ...
    const score = Math.max(
      0,
      Math.min(100, Math.round(onlinePct - alerts * 7)),
    );
    return {
      status,
      score,
      ...
      kpis: [
        {
          label: "KPCL online",
          value: `${summary?.kpcl_online_devices ?? 0}/${summary?.kpcl_total_devices ?? 0}`,
        },
        { label: "Offline >24h", value: `${offline24h}` },
        {
          label: "Eventos offline 24h",
          value: `${summary?.offline_events_last_24h ?? 0}`,
        },
      ],
    };
  }, [summary]);
  ...
  const auditSectionStatus = useMemo<SectionStatus>(() => {
    ...
    const score = Math.max(
      0,
      Math.min(100, 100 - pending * 4 - stalled24h * 8 - failedAudit * 3),
    );
    return {
      status,
      score,
  ...
  const infraSectionStatus = useMemo<SectionStatus>(() => {
    ...
    const score = Math.max(
      0,
      Math.min(100, 100 - bridgeOffline * 25 - Math.max(0, dbUsage - 70)),
    );
    return {
      status,
      score,
      ...
      kpis: [
        {
          label: "Bridge activo",
          value: `${summary?.bridge_active ?? 0}/${summary?.bridge_total ?? 0}`,
        },
        { label: "Uso Supabase", value: `${dbUsage}%` },
        {
          label: "DB total",
          value: `${supabaseStorage?.used_mb ?? 0}MB/${supabaseStorage?.plan_mb ?? 0}MB`,
        },
      ],
    };
  ...
  const financeSectionStatus = useMemo<SectionStatus>(() => {
    const avgMb = kpclFinancialRows.length
      ? kpclFinancialRows.reduce((acc, row) => acc + row.mb28d, 0) /
        kpclFinancialRows.length
      : 0;
    const opex =
      financeBreakdown?.total_cost_usd ?? financeSummary?.total_cost_usd ?? 0;
    ...
    const overheadUnit =
      unitsProduced > 0
        ...
        : null;
    const alerts =
      (avgMb > 10 ? 1 : 0) + (opex > 50 ? 1 : 0) + (warranty > 0 ? 1 : 0);
    const status: SectionStatus["status"] =
      opex > 50 ? "critical" : avgMb > 10 || warranty > 0 ? "warning" : "ok";
    const score = Math.max(
      0,
      Math.min(100, Math.round(100 - avgMb * 3 - alerts * 10)),
    );
    return {
      status,
      score,
      alerts,
      updatedAt:
        financeBreakdown?.updated_at ?? financeSummary?.generated_at ?? null,
      action:
        status === "critical"
      ...
      kpis: [
        ...
        {
          label: "BOM unitario",
          value: `USD ${(financeSummary?.bom_unit_cost_usd ?? 0).toFixed(2)}`,
        },
        {
  ...
  const testsSectionStatus = useMemo<SectionStatus>(() => {
    ...
    const lastRunAt =
      lastTestRun?.generated_at ?? testHistory[0]?.created_at ?? null;
    const staleRun =
      lastRunAt !== null
        ? Date.now() - Date.parse(lastRunAt) > 24 * 60 * 60 * 1000
        : true;
    const alerts = (failed > 0 ? 1 : 0) + (staleRun ? 1 : 0);
    const status: SectionStatus["status"] =
      failed > 0 ? "critical" : staleRun ? "warning" : "ok";
    const score =
      total > 0
        ? Math.max(0, Math.round(((total - failed) / total) * 100))
        : 80;
    return {
      status,
      score,
    ...
    const completionPct =
      (registrationSummary?.total_profiles ?? 0) > 0
        ? ((registrationSummary?.completed ?? 0) /
          ...
          100
        : 0;
    const conversionFreemium = Math.max(
      0,
      Math.round(completionPct * 0.15 * 10) / 10,
    );
  ...
  const scalingMetrics = useMemo(() => {
    ...
    const opexMonthly =
      financeBreakdown?.total_cost_usd ?? financeSummary?.total_cost_usd ?? 0;
    const marginIncremental =
      opexMonthly > 0 ? (costPerActive * 100) / opexMonthly : 0;
    ...
    financeBreakdown?.total_cost_usd,
    financeSummary?.total_cost_usd,
  ]);
  ...
  const saasValuation = useMemo(() => {
    ...
    const mrr = subscriptionPrice
      ? premiumUsersProxy * subscriptionPrice
      : null;
    ...
    const arpuFreemium =
      subscriptionPrice && activeUsersProxy > 0
        ...
        : null;
    const ltv =
      arpuFreemium !== null && churn && churn > 0
        ? arpuFreemium * (1 / churn)
        : null;
    ...
    const ev6 =
      mrr6 !== null && saasMultiple !== null ? mrr6 * 12 * saasMultiple : null;
    const ev12 =
      mrr12 !== null && saasMultiple !== null
        ? mrr12 * 12 * saasMultiple
        : null;
  ...
  const runAllAdminTests = async () => {
    ...
    if (permissions && !permissions.can_run_test_suite) {
      setTestRunnerMessage("Tu rol no permite ejecutar la suite de tests.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          payload?.error ?? "No se pudo ejecutar la suite de tests.",
        );
      }
      ...
      if (historyRes.ok) {
        ...
        setTestHistory(
          (historyPayload.history ?? []) as AdminTestHistoryItem[],
        );
      }
    } catch (error) {
      setTestRunnerMessage(
        error instanceof Error
          ? error.message
          : "Error no controlado en test runner.",
      );
    } finally {
  ...
  return (
    <div className="admin-density-compact min-h-screen bg-[radial-gradient(circle_at_top,_rgba(226,232,240,0.7),_rgba(248,250,252,1))] px-3 py-4 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <div className="mx-auto w-full max-w-[1400px] lg:flex lg:items-start lg:gap-5">
        <aside className="surface-card freeform-rise sticky top-4 hidden h-[calc(100vh-2rem)] w-72 shrink-0 overflow-auto px-3 py-3 lg:block">
          ...
          <nav className="mt-2 flex flex-col gap-1 text-sm">
            <a
              href="#admin-resumen"
              className="rounded-[var(--radius)] px-3 py-2 font-semibold text-slate-700 hover:bg-slate-100"
            >
              Resumen Admin
            </a>
            <a
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4 lg:gap-5">
          <nav className="surface-card freeform-rise flex gap-2 overflow-x-auto px-3 py-3 lg:hidden">
            <a
              href="#admin-resumen"
          ...
          <header
            id="admin-resumen"
            className="surface-card freeform-rise z-20 px-4 py-3 sm:px-6 sm:py-4 backdrop-blur"
          >
          ...
          {loading ? (
            <section className="surface-card freeform-rise px-4 py-5 text-sm text-slate-500 sm:px-6">
          ...
          {!loading && !error && summary ? (
            <>
              <AvisosCriticosCard
                ...
                canRunHealthCheck={
                  permissions ? permissions.can_run_health_check : true
                }
                onRunHealthCheck={runHealthCheck}
                criticalAlerts={criticalAlerts}
              />

              <KpiEjecutivosCard kpis={executiveKpis} />
              ...
              <div className="flex flex-col gap-6">
                <section
                  id="admin-operacion"
                  className="surface-card freeform-rise order-0 px-4 py-4 sm:px-6 sm:py-5"
                >
                  ...
                  permissions?.can_view_finance ? (
                    <section className="surface-card freeform-rise order-12 px-4 py-4 sm:px-6 sm:py-5">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        FX: 1 USD ≈ {formatClp(fxClpPerUsd)} ·{" "}
                        {formatJpy(fxJpyPerUsd)}
                      </p>
                      <SectionStatusCard
                        title="Finanzas Operativas"
                        data={financeSectionStatus}
                      />
                      <div className="mt-4 grid gap-3 md:grid-cols-4">
                        <div className="rounded-[var(--radius)] border border-slate-200 bg-white px-4 py-3">
                          ...
                          <p className="mt-2 text-xl font-semibold text-slate-900">
                            USD{" "}
                            {(financeSummary?.bom_unit_cost_usd ?? 0).toFixed(
                              2,
                            )}
                          </p>
                        </div>
                        <div className="rounded-[var(--radius)] border border-slate-200 bg-white px-4 py-3">
                          ...
                          <p className="mt-2 text-xl font-semibold text-slate-900">
                            USD{" "}
                            {(
                              financeSummary?.cloud_monthly_cost_usd ?? 0
                            ).toFixed(2)}
                          </p>
                        </div>
                        <div className="rounded-[var(--radius)] border border-slate-200 bg-white px-4 py-3">
                          ...
                          <p className="mt-2 text-xl font-semibold text-slate-900">
                            USD{" "}
                            {(financeSummary?.total_cost_usd ?? 0).toFixed(2)}
                          </p>
                        </div>
                        <div className="rounded-[var(--radius)] border border-slate-200 bg-white px-4 py-3">
                          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">
                          ...
                          <p className="mt-2 text-xl font-semibold text-slate-900">
                            USD{" "}
                            {(financeSummary?.unit_cost_usd ?? 0).toFixed(2)}
                          </p>
                          <p className="mt-1 text-xs text-slate-500">
                            {formatClp(
                              (financeSummary?.unit_cost_usd ?? 0) *
                                fxClpPerUsd,
                            )}{" "}
                            ·{" "}
                            {formatJpy(
                      ...
                      <div className="mt-3 rounded-[var(--radius)] border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
                        <p className="font-semibold text-slate-700">
                          Break-even (modelo operativo)
                        </p>
                        <p className="mt-1">
                          Costos fijos mensuales usados: USD{" "}
                          {(financeBreakEven?.fixed_monthly_usd ?? 0).toFixed(
                            2,
                          )}{" "}
                          · Precio plato:{" "}
                          {financeBreakEven?.plate_price_usd
                            ? `USD ${financeBreakEven.plate_price_usd.toFixed(2)}`
                            : "N/D"}{" "}
                          · Suscripción:{" "}
                            ...
                            {financePlans.length ? (
                              financePlans.map((plan) => (
                                ...
                                >
                                  ...
                                  <td className="px-2 py-2">
                                    USD{" "}
                                    {Number(plan.monthly_cost_usd ?? 0).toFixed(
                                      2,
                                    )}
                                  </td>
                                  <td className="px-2 py-2">
                        ...
                        <div className="rounded-[var(--radius)] border border-slate-200 bg-white p-3">
                          <div className="flex flex-wrap items-end gap-3">
                            <label className="text-xs font-semibold text-slate-600">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  setSelectedKpclId(e.target.value)
                                }
                          ...
                          <div className="mt-3 grid gap-2 md:hidden">
                            <div className="rounded-[10px] border border-slate-200 bg-slate-50 px-3 py-2 text-xs">
                              <p className="font-semibold text-slate-700">
                                Costo unitario componentes: USD{" "}
                                {selectedKpclCost.componentsTotal.toFixed(2)}
                              </p>
                              <p className="text-slate-500">
                            ...
                            <div className="rounded-[10px] border border-slate-200 bg-white px-3 py-2 text-xs">
                              <p className="text-slate-600">
                                MB 28d:{" "}
                                {selectedKpclRuntimeSim.deviceMb.toFixed(2)} ·
                                Opex mensual simulado: USD{" "}
                                {selectedKpclRuntimeSim.monthlyOpsUsd.toFixed(
                                  2,
                                )}
                              </p>
                            </div>
                          ...
                          <div className="mt-3 hidden overflow-x-auto md:block">
                            <table className="min-w-full text-left text-xs text-slate-600">
                              ...
                              <tbody>
                                {selectedKpclCost.effectiveComponents.map(
                                  (c) => (
                                    <tr
                                      key={c.code}
                                      className="border-b border-slate-100"
                                    >
                                      ...
                                      <td className="px-2 py-2">
                                        {c.unit_cost_usd.toFixed(2)}
                                      </td>
                                      <td className="px-2 py-2">
                                        {(c.qty * c.unit_cost_usd).toFixed(2)}
                                      </td>
                                      <td className="px-2 py-2">
                                        {formatClp(
                                          c.qty * c.unit_cost_usd * fxClpPerUsd,
                                        )}
                                      </td>
                                    </tr>
                                  ),
                                ...
                                <tr className="bg-slate-50">
                                  <td
                                    ...
                                    colSpan={3}
                                  >
                                    Costo componentes por unidad (
                                    {selectedKpclCost.has3dPrint
                                      ? "con impresión 3D"
                                      : "sin impresión 3D"}
                                    )
                                  </td>
                                  <td className="px-2 py-2 font-semibold text-slate-900">
                                    USD{" "}
                                    {selectedKpclCost.componentsTotal.toFixed(
                                      2,
                                    )}
                                  </td>
                                  <td className="px-2 py-2 font-semibold text-slate-900">
                                ...
                                <tr>
                                  <td
                                    ...
                                    colSpan={5}
                                  >
                                    Impresión: {selectedKpclCatalog.print_grams}
                                    g · {selectedKpclCatalog.print_hours}h · USD{" "}
                                    {selectedKpclCatalog.print_unit_cost_usd.toFixed(
                                      2,
                                    )}{" "}
                                    por unidad ·{" "}
                                    {formatClp(
                                      selectedKpclCatalog.print_unit_cost_usd *
                                        fxClpPerUsd,
                                    )}
                                    .
                                  </td>
                                ...
                                <tr>
                                  <td
                                    ...
                                    colSpan={5}
                                  >
                                    MB en ventana real (28d):{" "}
                                    {selectedKpclRuntimeSim.deviceMb.toFixed(2)}{" "}
                                    MB de{" "}
                                    {selectedKpclRuntimeSim.totalMb.toFixed(2)}{" "}
                                    MB totales KPCL.
                                  </td>
                                </tr>
                                <tr>
                                  <td
                                    ...
                                    colSpan={5}
                                  >
                                    Costo simulado HiveMQ por KPCL: USD{" "}
                                    {selectedKpclRuntimeSim.hivemqUnitUsd.toFixed(
                                      3,
                                    )}{" "}
                                    / mes
                                    {" · "}
                                    Costo simulado Vercel por KPCL: USD{" "}
                                    {selectedKpclRuntimeSim.vercelUnitUsd.toFixed(
                                      3,
                                    )}{" "}
                                    / mes
                                  </td>
                                ...
                                <tr>
                                  <td
                                    ...
                                    colSpan={5}
                                  >
                                    Mantenimiento+energía base: USD{" "}
                                    {selectedKpclCost.monthlyRuntimeUsd.toFixed(
                                      2,
                                    )}{" "}
                                    / mes
                                    {" · "}
                                    ...
                                    / mes (
                                    {formatClp(
                                      selectedKpclRuntimeSim.monthlyOpsUsd *
                                        fxClpPerUsd,
                                    )}
                                    ).
                                  </td>
                          ...
                          <div className="mt-3 rounded-[10px] border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] text-slate-500">
                            Filamento 3D de referencia:{" "}
                            {PLA_FILAMENT_PURCHASE.product} · proveedor{" "}
                            {PLA_FILAMENT_PURCHASE.supplier} · costo compra{" "}
                            {formatClp(PLA_FILAMENT_PURCHASE.source_price_clp)}{" "}
                            / {PLA_FILAMENT_PURCHASE.source_weight_grams}g (≈{" "}
                            {formatClp(
                              PLA_FILAMENT_PURCHASE.source_price_clp /
                                PLA_FILAMENT_PURCHASE.source_weight_grams,
                            )}{" "}
                            por gramo).
                          </div>
                          <div className="mt-4 hidden overflow-x-auto md:block">
                            <table className="min-w-full text-left text-xs text-slate-600">
                              <thead>
                                <tr className="border-b border-slate-200 text-slate-400">
                                  ...
                                  <th className="px-2 py-2 font-semibold">
                                    Costo unidad (USD/CLP)
                                  </th>
                                  <th className="px-2 py-2 font-semibold">
                                    MB 28d
                                  </th>
                                  <th className="px-2 py-2 font-semibold">
                                    Opex mensual (USD/CLP)
                                  </th>
                                </tr>
                              </thead>
                              <tbody>
                                {kpclFinancialRows.map((r) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-2 py-2">
                                      USD {r.monthlyUsd.toFixed(2)} ·{" "}
                                      {formatClp(r.monthlyClp)}
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                          ...
                          <div className="mt-4 grid gap-2 md:hidden">
                            {kpclFinancialRows.map((r) => (
                              ...
                              >
                                ...
                                <p className="text-slate-500">
                                  {r.model} ·{" "}
                                  {r.has3dPrint ? "3D: Sí" : "3D: No"}
                                </p>
                                <p className="text-slate-600">
                                  Unidad: USD {r.unitUsd.toFixed(2)} · 28d:{" "}
                                  {r.mb28d.toFixed(2)} MB
                                </p>
                              </div>
                            ))}
                          </div>
                    ...
                    <section className="surface-card freeform-rise order-12 px-4 py-4 sm:px-6 sm:py-5">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Tu rol no tiene acceso a información financiera.
                      </p>
                    </section>
                  )
                ) : null}
                    ...
                    <div>
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            Eje Y: dispositivos KPCL · Eje X: ventana de 1 hora
                            (estado real por hora).
                          </p>
                          <p className="mt-1 text-[11px] text-slate-400">
                            Tabla derecha: Hora total y MB total por KPCL (28d),
                            más última conexión.
                          </p>
                      ...
                      <div className="mt-4 rounded-[var(--radius)] border border-slate-200 bg-white p-3">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setHistoryOffsetHours((prev) =>
                                Math.min(
                                  prev + continuityChart.windowSpanHours,
                                  continuityChart.maxOffset,
                                ),
                              )
                            }
                            disabled={
                              continuityChart.safeOffset >=
                              continuityChart.maxOffset
                            }
                        ...
                        <div className="md:hidden space-y-2">
                          {continuityChart.rows.map((row) => {
                            const totalHours =
                              continuityChart.totalOnlineHoursByDevice.get(
                                row.device_id,
                              ) ?? 0;
                            const totalMb =
                              continuityChart.totalDataMbByDevice.get(
                                row.device_id,
                              ) ?? 0;
                            const lastSeen =
                              continuityChart.lastSeenByDevice.get(
                                row.device_id,
                              ) ?? "-";
                            const isOnline =
                              row.points[row.points.length - 1]?.online ===
                              true;
                            return (
                              <div
                                key={`m-cont-${row.device_id}`}
                                className="rounded-[10px] border border-slate-200 bg-white px-3 py-2 text-xs"
                              >
                                ...
                                <p className="text-slate-600">
                                  {totalHours}h · {totalMb.toFixed(2)}MB
                                </p>
                                <p className="text-slate-500">
                        ...
                        >
                          <rect
                            ...
                            height={
                              Math.max(1, continuityChart.rows.length) *
                              continuityChart.rowGap
                            }
                          ...
                          <line
                            x1={
                              ...
                              continuityChart.plotWidth +
                              continuityChart.sideGap / 2
                            }
                          ...
                          {continuityChart.xTicks.map((hourIndex) => {
                            const x =
                              continuityChart.left +
                              (hourIndex - continuityChart.startIndex) *
                                continuityChart.hourWidth;
                            ...
                            const yEnd =
                              ...
                              Math.max(1, continuityChart.rows.length) *
                                continuityChart.rowGap;
                            const ts =
                              continuityChart.referencePoints[hourIndex]?.ts;
                            const label =
                              ts !== undefined
                          ...
                          {continuityChart.rows.map((row, i) => {
                            const y =
                              continuityChart.top +
                              i * continuityChart.rowGap +
                              continuityChart.rowHeight;
                            return (
                              <g key={row.device_id}>
                                <text
                                ...
                                {row.points.slice(1).map((point, idx) => {
                                  ...
                                  if (idx < continuityChart.startIndex)
                                    return null;
                                  const x1 =
                                    continuityChart.left +
                                    (idx - continuityChart.startIndex) *
                                      continuityChart.hourWidth;
                                  const x2 =
                                    continuityChart.left +
                                    (idx + 1 - continuityChart.startIndex) *
                                      continuityChart.hourWidth;
                                  const stroke =
                      ...
                      <div className="mt-3 rounded-[var(--radius)] border border-slate-100 bg-slate-50/60 px-3 py-2">
                        ...
                        <div className="mt-3 border-t border-slate-200 pt-2">
                          ...
                          <p className="text-[11px] text-slate-500">
                            {supabaseStorage?.used_mb ?? 0} MB /{" "}
                            {supabaseStorage?.plan_mb ?? 0} MB
                          </p>
                ...
                {infraExpanded ? (
                  ...
                  <section className="surface-card freeform-rise order-11 px-4 py-3 sm:px-6 sm:py-4">
                    <p className="text-xs text-slate-500">
                      Infraestructura colapsada. Actívala desde el header para
                      ver telemetría técnica completa.
                    </p>
                  </section>
                )}

                <AuditoriaCard
                  auditSectionStatus={auditSectionStatus}
                  registrationSummary={registrationSummary}
                />

                <InfraestructuraCard
                  infraSectionStatus={infraSectionStatus}
                  infraExpanded={infraExpanded}
                  bridges={bridges}
                  kpclDevices={kpclDevices}
                />

                <TestsAdminCard
                  testsSectionStatus={testsSectionStatus}
                  testRunnerMessage={testRunnerMessage}
                  runAllAdminTests={runAllAdminTests}
                  testsRunning={testsRunning}
                  canRunTestSuite={
                    permissions ? permissions.can_run_test_suite : true
                  }
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setAuditWindowMin(Number(event.target.value))
                        }
                        className="h-8 rounded-[var(--radius)] border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-600"
