...
export const useDayCycleData = (
  ...
  return useMemo(() => {
    ...
    const startEvents = auditEvents.filter(
      (e) =>
        (e.category_key === "inicio_comida" ||
          e.category_key === "inicio_hidratacion") &&
        e.recorded_at >= startMs &&
        e.recorded_at <= endMs,
    );
    ...
    startEvents.forEach((evt) => {
      ...
      const termEvt = auditEvents.find(
        (e) => e.category_key === termKey && e.recorded_at > evt.recorded_at,
      );

      // Buscar peso inicial en readings
      const readingAtStart = readings.find(
        (r) => Math.abs(r.t - evt.recorded_at) < 30000,
      );
      const readingAtEnd = termEvt
        ? readings.find((r) => Math.abs(r.t - termEvt.recorded_at) < 30000)
        : null;

      if (readingAtStart) {
        const consumed = readingAtEnd
          ? Math.max(0, readingAtStart.y - readingAtEnd.y)
          : 10; // Fallback si no hay fin
        sessions.push({
          id: `session-${evt.id}`,
          ...
          consumed: consumed,
          isConfirmed: true,
          durationMinutes: termEvt
            ? (termEvt.recorded_at - evt.recorded_at) / 60000
            : 5,
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (diff > 5 && timeDiff < 300000) {
        const isNearAudit = sessions.some(
          (s) => Math.abs(s.startT - points[i - 1].raw.t) < 600000,
        );
