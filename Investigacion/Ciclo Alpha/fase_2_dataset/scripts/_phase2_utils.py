...
RESAMPLE_TARGET_S = 30  # Exp 09 — cadencia uniforme objetivo (segundos)
...
def compute_segment_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Exp 09B: plateau_duration en segundos (antes era conteo de filas)
    _plateau_rows = _group_transform(
        out,
        "is_plateau",
        lambda s: s.groupby((s != s.shift()).cumsum()).cumcount().add(1).mul(s),
    )
    out["plateau_duration"] = _plateau_rows * out["dt_seconds"]
    hour = out["ts"].dt.hour + out["ts"].dt.minute / 60 + out["ts"].dt.second / 3600
    out["hour_sin"] = np.sin(2 * np.pi * hour / 24)
    out["hour_cos"] = np.cos(2 * np.pi * hour / 24)
    # Exp 09B: cadencia_s como feature #13 — intervalo entre lecturas (clip 0-120s)
    out["cadencia_s"] = _group_transform(
        out,
        "ts",
        lambda s: s.diff().dt.total_seconds().clip(0, 120).fillna(RESAMPLE_TARGET_S),
    )
...
def resample_to_uniform(df: pd.DataFrame, target_s: int = RESAMPLE_TARGET_S) -> pd.DataFrame:
    """Resamplea lecturas a cadencia uniforme dentro de cada segmento de continuidad.

    Elimina el shift de distribución entre Abril 2026 (~14.7 s) y Mayo-Jun 2026
    (~30 s): con cadencia uniforme, rolling(5) y rolling(10) representan el mismo
    período en ambos datasets.

    Método: forward-fill (step function) — el peso del bowl no varía entre lecturas
    salvo por eventos de alimentación/servido, por lo que propagar el último valor
    conocido es la interpolación correcta.
    ...
    # Detectar segmentos de continuidad (mismo criterio que add_gap_segments)
    gap_s = df["ts"].diff().dt.total_seconds().fillna(0)
    seg   = (gap_s > GAP_CUTOFF_S).cumsum().astype(int)

    non_ts = [c for c in df.columns if c != "ts"]
    freq   = f"{target_s}s"
    parts: list[pd.DataFrame] = []

    for _, grp in df.groupby(seg, sort=True):
        grp = grp.set_index("ts").sort_index()

        # Primer punto de la grilla alineado al boundary superior del segmento
        t0 = grp.index[0].ceil(freq)
        t1 = grp.index[-1]

        if t0 > t1:
            # Segmento más corto que target_s — conservar tal cual
            parts.append(grp.reset_index())
            continue

        new_idx = pd.date_range(t0, t1, freq=freq, tz="UTC", name="ts")
        if new_idx.empty:
            parts.append(grp.reset_index())
            continue

        # Unir índice real + grilla uniforme, forward-fill, filtrar a la grilla
        merged = grp.reindex(grp.index.union(new_idx).sort_values())
        merged[non_ts] = merged[non_ts].ffill()
        merged = merged.loc[new_idx].dropna(subset=["weight_grams"])
    ...
    if "ts" not in out.columns:
        raise ValueError(f"Columna 'ts' no encontrada tras concat. Columnas: {out.columns.tolist()}")
    out = out.sort_values("ts").reset_index(drop=True)

    # pandas convierte bool a float64 cuando introduce NaN durante el reindex;
    # restaurar el dtype correcto
    if "clock_invalid" in out.columns:
        out["clock_invalid"] = out["clock_invalid"].fillna(False).astype(bool)
