...
def _batch_metricas_cached(anot_mtime: float, csv_mtime: float, _cats_dfs: dict, _df_lec: pd.DataFrame) -> dict:
    """Caché de _batch_metricas() por 5 min, keyed por mtime de anotaciones_av2.csv + CSVs
    crudos (args sin '_' → Streamlit sí los hashea; con '_' → excluidos del hash, solo valor).

    Sin esto, _batch_metricas recorre TODAS las anotaciones con calcular_metricas() (mask +
    filter sobre ~300k lecturas c/u) en cada rerun de Tab 2 (Analizar Curva) y Tab 4
    (Umbrales) — Streamlit re-ejecuta el script completo en cada click de cualquier widget
    dentro de esos tabs. Medido: ~4.4s sobre 814 anotaciones, repetido en 2 call sites.
    """
    return _batch_metricas(_df_lec, _cats_dfs)
...
div[data-testid="stRadio"][data-key="tab_nav"] label p { margin: 0; }
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        if len(df_anot) == 0:
            ...
            if cats_con_data:
                ...
                _mets_comp = _batch_metricas_cached(
                    ANOTACIONES_CSV.stat().st_mtime, _csv_max_mtime(), cats_con_data, df_lec,
                )
                rows = []
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        if len(df_anot) == 0 or df_lec is None:
            ...
            _stats_mets = _batch_metricas_cached(
                ANOTACIONES_CSV.stat().st_mtime, _csv_max_mtime(), cats_stats, df_lec,
            )
