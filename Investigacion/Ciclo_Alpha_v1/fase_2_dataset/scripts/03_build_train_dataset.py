...
FEATURE_COLS = [
    ...
    "hour_cos",
    "clock_invalid",
    "cadencia_s",  # Exp 09B: feature #13 — intervalo entre lecturas en segundos
]
...
def main() -> None:
    ...
    # Exp 09B: guardar ts_val para calibración de threshold por período en Fase 3
    ts_val = df_val[["ts"]].reset_index(drop=True)
    ts_val.to_parquet(TRAIN_DIR / "ts_val.parquet", index=False)
    print(f"[OK] ts_val.parquet -> {len(ts_val):,} filas")
